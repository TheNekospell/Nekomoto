...
import {
    BACKEND, NEKOCOIN_ADDRESS,
    nekocoinContract,
    NEKOMOTO_ADDRESS,
    nekomotoContract, PRISM_ADDRESS,
    prismContract, SHARD_ADDRESS,
    shardContract,
    sign
} from "@/interface.js";
...
export default function Detail() {
    ...
    useEffect(() => {
        ...
        if (address) {
            ...
            shardContract.is_approved_for_all(address, NEKOMOTO_ADDRESS).then(result => {
                // console.log("nekocoin allowance: ", result)
    ...
    const empower = async () => {
        ...
        sign(account).then(({typedMessage, signature}) => {
            BACKEND.empowerChest(address, code, typedMessage, signature).then(result => {
    ...
    const claimOfInvitation = async () => {
        setWaiting(true)
        const {typedMessage, signature} = await sign(account)
        const result = await BACKEND.claimRewardOfInvitation(address, typedMessage, signature)
        console.log("result: ", result)
        setSuccess(message(result))
    }
    ...
    const upgradeAscend = async () => {
        ...
        if (prismAllowance < ascendUpgradePrism) {
            arr.push({
                ...
                calldata: CallData.compile({
                    ...
                    amount: cairo.uint256(ascendUpgradePrism)
                })
            })
        }
        if (nekocoinAllowance < ascendUpgradeNeko) {
            arr.push({
                ...
                calldata: CallData.compile({
                    spender: NEKOMOTO_ADDRESS,
                    amount: cairo.uint256(ascendUpgradeNeko)
                })
            })
        }
        arr.push({
                ...
                <Row gutter={{md: 0, lg: 16}}>
                    <Col xs={24} sm={24} lg={12} className="margin-top-16">
                        <InviteCard chestEmpower={addressInfo.ChestEmpower || []}
                                    ...
                                    openedChestCount={info?.chestCount} openedMasterChestCount={info?.masterChestCount}
                                    openChest={openChest}/>
                    </Col>
                    <Col xs={24} sm={24} lg={12} className="margin-top-16">
                        <ClaimedCard type="short" totalClaimed={addressInfo.TotalClaimed}
                                     totalMana={addressInfo.TotalMana}
                                     shard={addressInfo.TemporalShardIdList?.length} boost={addressInfo.Boost}
                                     lucky={lucky} startTime={addressInfo.Buff?.StartTime}
        ...
        <Col xs={24} sm={12}>
          <ClaimedCard type="short" />
                ...
                <Row gutter={{md: 0, lg: 16}}>
                    ...
                    <Col xs={24} sm={24} lg={9} className="margin-top-16">
                        <EmptyCard>
                            <Row>
                                <Col xs={24} sm={24}>
                                    <Flex justify="space-between">
                                        ...
                                        <div
                                            className="card-little-title">{Math.round(Number(addressInfo.Buff?.Boost || 0) * 100) + "%"}</div>
                                    </Flex>
                                </Col>
                            ...
                            <Row style={{flex: 1}}>
                                ...
                                >
                                    <Button text={"UP TO " + getAscendUpgrade(addressInfo.Buff?.Level) + "%"}
                                            color="yellow"
                                            longness="short" onClick={upgradeAscend}/>
                                </Col>
                            </Row>
                        </EmptyCard>
                    </Col>
                    <Col xs={24} sm={24} lg={9} className="margin-top-16">
                        <EmptyCard>
                            ...
                            <Flex justify="space-between" className="card-mini-title">
                                <div>Available</div>
                                <div>{Math.min(Number(addressInfo.InvitationReward?.TotalAmount) - Number(addressInfo.InvitationReward?.ClaimedAmount), Number(addressInfo.InvitationReward?.UnlockedAmount))}</div>
                            </Flex>
                            <Flex justify="space-between" className="card-mini-title">
                                <div>Locked</div>
                                <div>{Number(addressInfo.InvitationReward?.TotalAmount) - Number(addressInfo.InvitationReward?.ClaimedAmount)}</div>
                            </Flex>
                            <Row style={{flex: 1}}>
                                <Col
                                    xs={24} sm={24}
                                    ...
                                    style={{alignSelf: "flex-end", marginTop: "24px"}}
                                >
                                    <Button text="Claim" color="yellow" longness="short" onClick={claimOfInvitation}/>
                                </Col>
                            </Row>
                        </EmptyCard>
                    </Col>
                ...
                <div className="cards-wrapper margin-top-16">
                    ...
                    <div
                        className="card-title">{"My Neko (" + addressInfo.NekoSpiritList?.filter((item) => item.IsStaked === true).length + "/" + addressInfo.NekoSpiritList?.length + ")"}</div>
                    <Flex
                        style={{margin: "24px 0"}}
                    ...
                    >
                        {isMobile ? (
                            ...
                            <div>
                                <RadioButton text="all" active={nekoButton === "all"}
                                             onClick={() => setNekoButton("all")}/>
                                <RadioButton text="LEGENDARY" active={nekoButton === "LEGENDARY"}
                                             onClick={() => setNekoButton("LEGENDARY")}/>
                                <RadioButton text="EPIC" active={nekoButton === "EPIC"}
                                             onClick={() => setNekoButton("EPIC")}/>
                                <RadioButton text="RARE" active={nekoButton === "RARE"}
                                             onClick={() => setNekoButton("RARE")}/>
                                <RadioButton text="UNCOMMON" active={nekoButton === "UNCOMMON"}
                                             onClick={() => setNekoButton("UNCOMMON")}/>
                                <RadioButton text="COMMON" active={nekoButton === "COMMON"}
                        ...
                        <Flex>
                            <div className="card-desc-title"
                                 ...
                                 onClick={stakeAll}>Stake All
                            </div>
                            <div className="card-desc-title" style={{fontSize: '12px', cursor: 'pointer'}}
                                 onClick={unStakeAll}>UnStake All
                            </div>
                        </Flex>
                    </Flex>
                        ...
                        {chestDetail?.Token2Amount && (
                            <Col>
                                <div className="adept-bg">
                                    ...
                                    <div
                                        className="modal-text10">{"x" + chestDetail?.Token2Amount}</div>
                                </div>
                            </Col>
                        )}
                        ...
                        {chestDetail?.Token1Amount && (
                            <Col>
                                <div className="adept-bg">
                                    ...
                                    <div
                                        className="modal-text10">{"x" + chestDetail?.Token1Amount}</div>
                                </div>
                            </Col>
                        )}
                        ...
                        {chestDetail?.NFTAmount && (
                            <Col>
                                <div className="adept-bg">
                                    ...
                                    <div className="modal-text1">Neko</div>
                                    <div
                                        className="modal-text10">{"x" + chestDetail?.NFTAmount}</div>
                                </div>
                            </Col>
                        )}
                    </Row>
